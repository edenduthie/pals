...
public class GenerateBenchmarks extends UserAwareAction {

	static final Logger log = Logger.getLogger(GenerateBenchmarks.class);
	String message;
	...
	public String execute() {
		...
		if(!user.isAdmin())
		{
			message = "Only administrators can generate benchmarks";
			return ERROR;
		}
		log.info("Starting rerun of all benchmarks");
		Thread t = new GenerateBenchmarksRunner(user,dataSetVersionService);
		t.start();
		return SUCCESS;
	}
	
	public class GenerateBenchmarksRunner extends Thread
	{
         DataSetVersionService dataSetVersionService;
         User user;
         
         public GenerateBenchmarksRunner(User user, DataSetVersionService dataSetVersionService)
         {
        	 this.user = user;
         ...
         public void run()
         {
        	dataSetVersionService.generateBenchmarks(user);
         }
	}
