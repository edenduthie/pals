...
datasetname='mitra2'
fluxnetdir = '~/data/fluxnet/LaThuile_open/PT-Mi2'
...
for(f in 1:length(fnet)){
	...
	if(f == 1){ 
		...
		# Start of data:
		sdate = doydate(as.integer(fdata[1,2]),leap=is.leap(as.integer(fdata[1,1])))
...
system(paste('r -f  ~/Documents/admin/PALS/scripts/working/pals-scripts/ConvertSpreadsheetToNcdf.R',
	outfile,' ~/Documents/admin/PALS/testing/TestMet.nc',
	'~/Documents/admin/PALS/testing/TestFlux.nc -35.78024',
	'148.00781 1200.0 70.0 testTest 0.11'))

cresult = readline(prompt = 'Conversion okay?')
if(cresult != ''){stop()}

# Generate and view QC plots:
system('r -f ~/Documents/admin/PALS/scripts/working/pals-scripts/QCplotsSpreadsheet.R ~/Documents/admin/PALS/testing/TestMet.nc ~/Documents/admin/PALS/testing/TestFlux.nc ~/Documents/admin/PALS/testing/testQC png 1200 2200')
system('open -a Preview.app ~/Documents/admin/PALS/testing/testQC.png')